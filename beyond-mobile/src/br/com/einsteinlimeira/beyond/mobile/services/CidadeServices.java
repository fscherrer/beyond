...
import br.com.einsteinlimeira.beyond.protocol.RequisicaoCidade;

public class CidadeServices {
	
	public List<Cidade> getCidade(Context contexto) throws IOException{
		...
		String host = sharedPreferences.getString(Constantes.CONFIGURACAO_HOST,
				contexto.getResources().getString(R.string.configuracao_host_padrao));
		...
		parametros.add(new BasicNameValuePair(Requisicao.PARAMETRO_TIPO,
				Requisicao.TIPO_CIDADE));
		parametros.add(new BasicNameValuePair(RequisicaoCidade.PARAMETRO_CIDADE,
				Requisicao.TODAS));
		...
		try{
			...
			if(statusCode != 200){
				...
				return new Gson().fromJson(json, new TypeToken<List<Cidade>>(){}.getType());
			}
